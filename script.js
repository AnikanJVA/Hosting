document.addEventListener('DOMContentLoaded', () => {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            if (formMessage) {
                ...
                formMessage.classList.add('opacity-100', 'bg-teal-100', 'text-black-800', 'text-center');
    ...
    function viewSubmissions() {
        ...
        list.innerHTML = data.reverse().map(entry => `
            <tr class="border-b border-gray-100 align-text-top">
                <td class="border-r border-gray-300 px-6 py-4 text-xs font-mono text-gray-400">${entry.submittedAt}</td>
                <td class="border-r border-gray-300 px-6 py-4 font-bold text-gray-800">${entry.name}</td>
                <td class="border-r border-gray-300 px-6 py-4 text-blue-600">${entry.contact_number}</td>
                <td class="border-r border-gray-300 px-6 py-4 text-blue-600">${entry.email}</td>
                <td class="border-r border-gray-300 px-6 py-4 text-gray-600">${entry.message}</td>
            </tr>
        `).join('');
    }
